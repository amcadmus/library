...
Integral3DInfo::Method Integral3DInfo::Simpson	= 0;
Integral3DInfo::Method Integral3DInfo::Gauss27	= 1;
...
template <typename TrinaryFunction, typename ValueType >
ValueType Integral3D<TrinaryFunction, ValueType >::
...
//   return exp(x*x+y*y+4*z*z);
}


int main ()
{
  Integral3D<F3, double > inte;
  double value = inte.cal_int 
      (Integral3DInfo::Gauss27, f, 0., M_PI, 0.0, M_PI, 0.0, M_PI, 1e-6);
  std::cout.precision (16);
  std::cout << value << std::endl;
  double tol = 1e-10;
  std::cout << std::scientific 
	    <<  inte.cal_int 
      (Integral3DInfo::Gauss27, f, 0., M_PI, 0.01, 0.5, 0.04, 0.1, tol) - value 
	    << std::endl;;
